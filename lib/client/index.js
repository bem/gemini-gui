...
function gotoSuiteCode(e) {
    ...
    xhr.get('/suite-code-position?suite=' + encodeURIComponent(suiteText), function (err, data) {
        if (err) {
            alert('Server is not responding. Try later.');
            return;
        }
        openWebStorm(data, function (result) {
            if (!result) { // ws not opened
                openGithub(data, e);
                alert('WebStorm is not startd\n'
...
function openWebStorm(data, callback) {
    xhr.get('http://' + location.hostname + ':63330/file?buildId=&file=' + data.file + '&line=' + data.line + '&noCache=' + (Math.random()*1e6) + '&server='+location.origin, function (err, res) {
        callback(!err);
    });
    return true;
}
